<script lang="ts">
	...
	const columns = [
		...
		{ title: 'Resources', links: ['Documentation', 'Support', 'Status', 'API'] },
	...
	const socialIconClass =
		'text-gray-500 transition-colors hover:text-gray-700 dark:text-gray-400 dark:hover:text-white';
</script>

<footer class="w-full border-t border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900">
	<div class="mx-auto flex max-w-(--breakpoint-2xl) flex-col gap-12 px-4 pt-16 pb-12 md:px-6">
		<div
			class="flex flex-wrap items-start justify-between gap-y-8 border-b border-gray-200 pb-12 dark:border-gray-800"
		>
			<div class="flex min-w-80 flex-1 flex-col gap-2">
				<p class="text-xl font-semibold text-gray-900 dark:text-white">Join our newsletter</p>
				<p class="text-theme-sm text-gray-500 dark:text-gray-400">
					We&rsquo;ll send you a nice letter once per week. No spam.
				</p>
			</div>
			<form class="flex w-full max-w-md items-start gap-3" onsubmit={(e) => e.preventDefault()}>
				<input
					type="email"
					bind:value={email}
		...
		<div class="flex flex-wrap gap-12">
			...
			<div class="flex flex-1 flex-wrap gap-8">
				{#each columns as column (column.title)}
					<nav class="flex min-w-32 flex-1 flex-col gap-4" aria-label={column.title}>
						<p class="text-theme-sm font-semibold text-gray-700 dark:text-gray-300">
							{column.title}
						</p>
						<ul class="flex flex-col gap-3">
							{#each column.links as label (label)}
								<li>
									<button
										type="button"
										class="text-left text-theme-sm font-medium text-gray-500 transition-colors hover:text-gray-700 dark:text-gray-400 dark:hover:text-white"
									>
										{label}
									</button>
								</li>
