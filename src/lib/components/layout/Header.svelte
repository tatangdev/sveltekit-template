<script lang="ts">
	...
	let notificationTrigger = $state<HTMLElement | null>(null);
	let userTrigger = $state<HTMLElement | null>(null);
	let searchInput = $state<HTMLInputElement | null>(null);

	onMount(() => {
		const onKey = (e: KeyboardEvent) => {
			const meta = e.metaKey || e.ctrlKey;
			if ((meta && e.key === 'k') || (e.key === '/' && document.activeElement !== searchInput)) {
				e.preventDefault();
				searchInput?.focus();
			}
		};
...
>
	<div class="flex grow flex-col items-center justify-between xl:flex-row xl:px-6">
		<div
			class="flex w-full items-center justify-between gap-2 border-b border-gray-200 p-3 sm:gap-4 lg:py-4 xl:justify-normal xl:border-b-0 xl:px-0 dark:border-gray-800"
		>
			<button
			...
			>
				...
				<svg
					class="fill-current xl:hidden {sidebar.toggle ? 'hidden' : 'block xl:hidden'}"
					width="24"
					height="24"
					viewBox="0 0 24 24"
					fill="none"
		...
		<div
			class="w-full items-center justify-between gap-4 px-5 py-4 shadow-theme-md xl:flex xl:justify-end xl:px-0 xl:shadow-none {menuToggle
				? 'flex'
				: 'hidden'}"
			use:clickOutside={() => (menuToggle = false)}
		>
			<div class="flex items-center gap-2 2xsm:gap-3">
				...
				<div class="relative">
					...
					>
						<span
							class="absolute top-0.5 right-0 z-1 h-2 w-2 rounded-full bg-orange-400 {!notifying
								? 'hidden'
								: 'flex'}"
						>
							<span
								...
								<li>
									...
									>
										<span class="relative z-1 block h-10 w-full max-w-10 rounded-full">
											...
											<span
												class="absolute right-0 bottom-0 z-10 h-2.5 w-full max-w-2.5 rounded-full border-[1.5px] border-white dark:border-gray-900 {n.online
													? 'bg-success-500'
													: 'bg-error-500'}"
											></span>
										</span>
			...
			<div class="relative">
				...
				>
					...
					<ChevronDownIcon
						class="stroke-gray-500 dark:stroke-gray-400 {userOpen ? 'rotate-180' : ''}"
					/>
				</button>
